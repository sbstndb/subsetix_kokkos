...
using V5Mesh3D = parallel_merge::Mesh3D<>;
...
void register_large_benchmarks_2d() {
  auto large_10k_a = mesh_generator::generate_dense_mesh<V5Mesh2D>(10000, 0);
  auto large_10k_b = mesh_generator::generate_dense_mesh<V5Mesh2D>(10000, 0);

  benchmark::RegisterBenchmark("2D_LARGE_10Kx10K_v5_parallel_merge_balanced",
      [&, mesh_a=std::move(large_10k_a), mesh_b=std::move(large_10k_b)](benchmark::State& state) {
    BM_RowMapping_Generic(state, "v5_parallel_merge_balanced",
        [](const auto& a, const auto& b) {
          return parallel_merge::intersect_meshes_2d(a, b);
        },
        mesh_a, mesh_b);
  })->Unit(benchmark::kMillisecond);

  auto large_10k_unbal_a = mesh_generator::generate_dense_mesh<V5Mesh2D>(10000, 0);
  auto large_1k_unbal_b = mesh_generator::generate_dense_mesh<V5Mesh2D>(1000, 0);

  benchmark::RegisterBenchmark("2D_LARGE_10Kx1K_v5_parallel_merge_unbalanced",
      [&, mesh_a=std::move(large_10k_unbal_a), mesh_b=std::move(large_1k_unbal_b)](benchmark::State& state) {
    BM_RowMapping_Generic(state, "v5_parallel_merge_unbalanced",
        [](const auto& a, const auto& b) {
          return parallel_merge::intersect_meshes_2d(a, b);
        },
        mesh_a, mesh_b);
...
void register_large_benchmarks_3d() {
  auto large_512_a = mesh_generator::generate_dense_mesh<V5Mesh3D>(512 * 512, 0);
  auto large_512_b = mesh_generator::generate_dense_mesh<V5Mesh3D>(512 * 512, 0);

  benchmark::RegisterBenchmark("3D_LARGE_512x512_v5_parallel_merge_balanced",
      [&, mesh_a=std::move(large_512_a), mesh_b=std::move(large_512_b)](benchmark::State& state) {
    BM_RowMapping_Generic(state, "v5_parallel_merge_3d_balanced",
        [](const auto& a, const auto& b) {
          return parallel_merge::intersect_meshes_3d(a, b);
        },
        mesh_a, mesh_b);
  })->Unit(benchmark::kMillisecond);

  auto large_1024_unbal_a = mesh_generator::generate_dense_mesh<V5Mesh3D>(1024 * 512, 0);
  auto large_512_unbal_b = mesh_generator::generate_dense_mesh<V5Mesh3D>(512 * 512, 0);

  benchmark::RegisterBenchmark("3D_LARGE_1024x512_v5_parallel_merge_unbalanced",
      [&, mesh_a=std::move(large_1024_unbal_a), mesh_b=std::move(large_512_unbal_b)](benchmark::State& state) {
    BM_RowMapping_Generic(state, "v5_parallel_merge_3d_unbalanced",
        [](const auto& a, const auto& b) {
          return parallel_merge::intersect_meshes_3d(a, b);
        },
        mesh_a, mesh_b);
