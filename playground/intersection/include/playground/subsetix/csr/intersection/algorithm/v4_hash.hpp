...
// ============================================================================
// hash_based Mesh type (identical to baseline, see baseline.hpp for full documentation)
// ============================================================================

/** @brief CSR mesh for hash-based algorithm. Identical to baseline::Mesh. */
...
/**
 * @brief Golden ratio hash constant for mixing.
 */
constexpr std::uint32_t GOLDEN_RATIO_HASH = 0x9e3779b9U;

/**
 * @brief Hash lookup for 2D (by y-coordinate).
 *
 * Uses linear probing with golden-ratio hash mixing.
 * Returns -1 if not found.
 */
template <class HashKeysView, class HashValuesView, class CoordType>
KOKKOS_INLINE_FUNCTION
int hash_lookup_2d(const HashKeysView& hash_keys,
                   const HashValuesView& hash_values,
                   std::size_t table_size,
                   CoordType y) {
  const std::size_t mask = table_size - 1;

  // Golden-ratio hash mixing
  std::uint32_t key = static_cast<std::uint32_t>(y);
  std::size_t idx = (key * GOLDEN_RATIO_HASH) & mask;

  const std::size_t start = idx;
  do {
    // Empty slot
    if (hash_values(idx) < 0) {
      return -1;
    }
    // Found match
    if (hash_keys(idx).y == y) {
...
/**
 * @brief Hash lookup for 3D (by y,z coordinates).
 *
 * Uses linear probing with golden-ratio hash mixing.
 * Combines y and z for the hash key.
 * Returns -1 if not found.
 */
template <class HashKeysView, class HashValuesView, class CoordType>
KOKKOS_INLINE_FUNCTION
int hash_lookup_3d(const HashKeysView& hash_keys,
                   const HashValuesView& hash_values,
                   std::size_t table_size,
                   CoordType y,
                   CoordType z) {
  ...
  // Combine y and z for hash (XOR with shift)
  std::uint32_t key = static_cast<std::uint32_t>(y) ^
                      (static_cast<std::uint32_t>(z) << 16);
  std::size_t idx = (key * GOLDEN_RATIO_HASH) & mask;
  ...
  do {
    ...
    // Found match (both y and z must match)
    if (hash_keys(idx).y == y && hash_keys(idx).z == z) {
      return hash_values(idx);
    }
    // Linear probe
    idx = (idx + 1) & mask;
...
// ============================================================================
// Mesh intersection (2D and 3D) - hash-based Algorithm
// ============================================================================

/**
 * @brief Mesh intersection using hash-based row mapping.
 *
 * Algorithm:
 * 1. Build hash table from mesh B's row keys (O(n) with collision handling)
 * 2. Query hash table for each row in A (expected O(1), worst O(n))
 * 3. Count - count intersecting X-intervals per row
 * 4. Scan - compute CSR offsets (row_ptr)
 * 5. Fill - write intersected intervals
 * 6. Compact - filter rows with no intersections
 *
 * The hash-based approach avoids O(log n) binary search for row mapping,
 * instead using O(1) expected-time hash lookups.
 *
...
intersect_meshes(const Mesh<DIM, Kokkos::DefaultExecutionSpace::memory_space, CoordType, IndexType>& A,
  ...
  // Allocate hash table with power-of-2 size for 0.7 load factor
  const std::size_t hash_table_size = detail::next_power_of_2(
      static_cast<std::size_t>(num_rows_b / 0.7) + 1);

  // Initialize hash table with -1 (empty marker)
  Kokkos::View<RowKey*, DeviceMemorySpace> hash_keys("hash_keys", hash_table_size);
  Kokkos::View<int*, DeviceMemorySpace> hash_values("hash_values", hash_table_size);

  // Initialize all slots as empty
  Kokkos::parallel_for(
      "hash_init",
      Kokkos::RangePolicy<ExecSpace>(0, hash_table_size),
  ...
  // Phase 1a: Build hash table from mesh B's row keys
  if constexpr (DIM == 2) {
    // 2D: hash by y only
    Kokkos::parallel_for(
        "hash_build_2d",
        ...
        KOKKOS_LAMBDA(const std::size_t i) {
          const RowKey key = rows_b(i);
          const std::size_t mask = hash_table_size - 1;

          // Golden-ratio hash
          std::uint32_t y_key = static_cast<std::uint32_t>(key.y);
          std::size_t idx = (y_key * detail::GOLDEN_RATIO_HASH) & mask;

          // Linear probing until empty slot
          while (hash_values(idx) >= 0) {
            idx = (idx + 1) & mask;
          }

          // Insert
          hash_keys(idx) = key;
          hash_values(idx) = static_cast<int>(i);
        });
  } else {
    // 3D: hash by (y,z)
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const std::size_t i) {
          ...
          // Combine y and z for hash
          std::uint32_t combined = static_cast<std::uint32_t>(key.y) ^
                                   (static_cast<std::uint32_t>(key.z) << 16);
          std::size_t idx = (combined * detail::GOLDEN_RATIO_HASH) & mask;
  ...
  // Allocate temporary arrays for row mapping
  ...
  // Phase 1b: Query hash table for each row in A
  if constexpr (DIM == 2) {
    // 2D: query by y only
    Kokkos::parallel_for(
        "hash_query_2d",
        ...
        KOKKOS_LAMBDA(const std::size_t i) {
          ...
          const int idx_b = detail::hash_lookup_2d(
              hash_keys, hash_values, hash_table_size, key.y);
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(const std::size_t i) {
          const RowKey key = rows_a(i);
          const int idx_b = detail::hash_lookup_3d(
              hash_keys, hash_values, hash_table_size, key.y, key.z);
