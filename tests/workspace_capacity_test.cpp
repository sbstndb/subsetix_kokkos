...
TEST(WorkspaceCapacityTest, AutomaticGrowth) {
  ...
  // Initial request
  auto buf1 = ws.get_int_buf_0(100);
  EXPECT_GE(buf1.extent(0), 100);
  EXPECT_EQ(ws.int_buf_0.extent(0), buf1.extent(0));

  // Request smaller size - should NOT shrink/reallocate
  auto ptr_before = ws.int_buf_0.data();
  auto buf2 = ws.get_int_buf_0(50);
  EXPECT_EQ(ws.int_buf_0.data(), ptr_before);
  EXPECT_GE(ws.int_buf_0.extent(0), 100);

  // Request larger size - should reallocate
  auto buf3 = ws.get_int_buf_0(200);
  EXPECT_GE(buf3.extent(0), 200);
  EXPECT_GE(ws.int_buf_0.extent(0), 200);
...
TEST(WorkspaceCapacityTest, MultipleBuffers) {
  ...
  auto ib0 = ws.get_int_buf_0(10);
  auto ib1 = ws.get_int_buf_1(20);
  auto st0 = ws.get_size_t_buf_0(30);

  EXPECT_GE(ws.int_buf_0.extent(0), 10);
  EXPECT_GE(ws.int_buf_1.extent(0), 20);
  EXPECT_GE(ws.size_t_buf_0.extent(0), 30);
}
...
TEST(WorkspaceCapacityTest, ClearReclaimsMemory) {
  ...
  ws.get_int_buf_0(1000);
  ws.get_row_key_buf_0(500);
  
  EXPECT_GE(ws.int_buf_0.extent(0), 1000);
  EXPECT_GE(ws.row_key_buf_0.extent(0), 500);
  ...
  EXPECT_EQ(ws.int_buf_0.extent(0), 0);
  EXPECT_EQ(ws.row_key_buf_0.extent(0), 0);
